...
class Solver(object):
    ...
    def run(self):
        ...
        # this technique leads to a memory error, so consume less
        self.solved = [[0] * self.n for i in range(self.t)]
        self._solve_rows()
        return self.solved[-1][-1] % MAX_VAL
    ...
    def _solve_row(self, i):
        self.solved[i][:] = self.solved[i - 1]
        for j in range(1, self.n - 1):
            self.solved[i][j] += self.solved[i - 1][j - 1]
            self.solved[i][j] += self.solved[i - 1][j + 1]
        self.solved[i][-1] += self.solved[i - 1][-2]
